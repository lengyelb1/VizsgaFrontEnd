    ...
    function CommentsKi (params) {
        if (params.post != null) {
            return params.post.comments.map((comment) => (
                <div key={params.post.id+(comment.id+1)} className='card card-green col-12 d-inline-block m-1 p-1 '>
        ...
        return params.posts.map((post) => (
            <div key={post.id + 1} className='card col-md-5 p-2 bg-dark text-light mx-auto mt-3 border border-dark shadow-green'>
                ...
                <div className='text-green align-middle'>
                    ...
                    <button className='btn rounded'>
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#A8F231" class="bi bi-fire" viewBox="0 0 16 16">
        ...
        <div className='mx-auto'>
            <nav class="navbar navbar-dark bg-dark w-100">
              <div class="container-fluid">
                <a class="nav-link text-light" href="/" onClick={()=>{
                    localStorage.setItem("token",undefined)
                }}>Log Out</a>
              </div>
            </nav>
            <div className='row w-100 mx-auto p-2'>
                {posts.map((post) => (
                    <div key={post.id + 1} className='card col-md-5 p-2 bg-dark text-light mx-auto border border-dark shadow'>
                        <h5 className='text-muted'>{post.userId}</h5>
                        <h5 className=''>{post.title}</h5>
                        <div className='card-body'>
                            <div className='small'>{post.description}</div>
                        </div>
                        <div className='text-muted'>{post.like}</div>
                        <button className='btn rounded'>
