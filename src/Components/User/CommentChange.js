...
export default function CommentChange(){
    ...
    useEffect(()=>{
        ...
        fetch(`${url}/Comment/CommentById?id=${params.id}`,{method:"GET",headers:{'Authorization': `Bearer ${localStorage.getItem('token')}`}})
        .then((r)=>
            r.json()
        )
        .then((resp)=>{
        ...
        <div className={`mx-auto text-center ${localStorage.getItem("darkMode")==0? "bg-light text-dark":"bg-dark text-green"} vh-100 col-12`}>
            ...
            <form onSubmit={
                (e)=>{
                    ...
                    fetch(`${url}/Comment/ChangeOwnComment?id=${params.id}&userId=${jwtDecode(localStorage.getItem("token")).id}`,{
                ...
                <textarea className="input-green w-50" id="text" name="text" placeholder="Text" defaultValue={data?.text}/>
                <br/>
