    ...
    return(
        <div className={`min-vh-100 ${localStorage.getItem("darkMode")==0? "bg-light text-dark":"bg-dark text-light"}`}>
            ...
            <div className="mx-auto text-center">
                <h1>Are you sure about deleteing this comment?</h1>

                <div className="d-flex justify-content-center gap-2">
                    <a type="button" className="btn btn-secondary" onClick={async ()=>{await navigate(-1)}}>Cancel</a>
                    <button type="button" className="btn btn-danger" onClick={async() =>{fetch(`${url}/Comment/DeleteCommentByUserId?id=${prop.id}&userId=${jwtDecode(localStorage.getItem("token")).id}`,{method:"DELETE",headers:{'Authorization': `Bearer ${localStorage.getItem('token')}`}}); alert("Successfull delete!"); refreshDatas(refrDatas+1);await navigate(-1); }}>Delete</button>
                </div>
            </div>
        </div>
    )
